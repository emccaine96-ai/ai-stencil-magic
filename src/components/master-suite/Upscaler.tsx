...
export function Upscaler({ photo, onReplace }: Props) {
  ...
  async function run() {
    ...
    try {
      ...
      const w = new Worker(new URL("../../lib/lanczos.worker.ts", import.meta.url), {
        type: "module",
      });
      workerRef.current = w;
      w.onmessage = (e: MessageEvent<{ type: string; p?: number; image?: ImageData }>) => {
        ...
        if (e.data.type === "done" && e.data.image) {
          ...
          out.width = e.data.image.width;
          out.height = e.data.image.height;
    ...
    <div className="space-y-3 text-sm">
      ...
      >
        {progress !== null && progress < 100 ? (
          <>
            <Loader2 className="animate-spin" size={14} /> Upscaling {progress}%
          </>
        ) : (
          <>
            <Zap size={14} /> Run Lanczos-3 upscale
          </>
        )}
      </button>
      {progress !== null && progress < 100 ? (
      ...
      {resultUrl && resultSize ? (
        <div className="space-y-2">
          <div className="text-[11px] text-muted-foreground">
            Result: {resultSize.w}×{resultSize.h}px
          </div>
          <div className="grid grid-cols-2 gap-2">
            <button
              onClick={download}
              className="rounded-xl border border-border py-2 text-xs font-semibold hover:border-primary flex items-center justify-center gap-1"
            >
              <Download size={12} /> Download
            </button>
            <button
              onClick={() => onReplace(resultUrl)}
              className="rounded-xl bg-gradient-primary text-primary-foreground py-2 text-xs font-semibold"
            >
              Use as reference
            </button>
          </div>
      ...
      <p className="text-[10px] text-muted-foreground">
        Lanczos-3 (3-lobe sinc) resampling in a Web Worker. Capped at 4K to stay within browser
        memory.
      </p>
    </div>
  );
