...
export default function SkinViewport({ stencilUrl }: { stencilUrl: string | null }) {
  ...
  useEffect(() => {
    ...
    const SKIN_COLOR: Record<typeof skinTone, number> = {
      light: 0xf3d4b0,
      medium: 0xd99e7a,
      deep: 0x8b5a3c,
    };
    ...
    const tick = () => {
      ...
      if (decal) {
        decal.rotation.copy(mesh.rotation);
      }
    ...
    <div className="space-y-3 text-sm">
      ...
      <div>
        <div className="flex justify-between text-[10px]">
          <span className="text-muted-foreground">Tattoo size</span>
          <span className="font-bold gradient-text">{Math.round(scale * 100)}%</span>
        </div>
        <input
          type="range"
          min={20}
      ...
      <label className="flex items-center justify-between text-[10px]">
        ...
        <input
          ...
          className="accent-[oklch(0.64_0.26_303)]"
        />
      </label>
      <p className="text-[10px] text-muted-foreground">
        Drag to rotate manually. Stencil ink blends into the skin via physically-based shading.
      </p>
    </div>
  );
}
