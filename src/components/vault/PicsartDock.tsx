...
import {
  Crop, Sparkles, Wand2, Scissors, Type as TypeIcon, ImagePlus,
  Brush, Square, Shapes, Pen, Frame, MessageSquare, Grid3x3,
  FolderOpen, Shirt, Aperture, Star, Layers, SlidersHorizontal,
  Rocket, Eraser, Stamp, Droplet, Wind, Contrast, Thermometer,
  Circle, FlipHorizontal2, FlipVertical2, RotateCw, ZoomIn,
  PaintBucket, Lightbulb, ScanLine, Activity, Pipette, Hand,
  Maximize2, Sparkle, Wand, Layout, Gauge,
} from "lucide-react";
...
export function buildCategories(h: PicsartDockHandlers): DockCategory[] {
  return [
    {
      id: "tools", label: "Tools", icon: Crop,
      actions: [
    ...
    {
      id: "retouch", label: "Retouch", icon: Wand,
      actions: [
    ...
    {
      id: "add-photo", label: "Add Photo", icon: ImagePlus,
          ...
          <div className="grid grid-cols-4 gap-px bg-white/5">
            {active.actions.map(a => {
              ...
              return (
                ...
                >
                  {a.pro && (
                    <span className="absolute top-1 left-1/2 -translate-x-1/2 translate-x-4 px-1 py-px rounded-sm bg-[#A855F7] text-[7px] font-bold text-white">PRO</span>
                  )}
                  {a.ai && (
                    <span className="absolute top-1 left-1/2 -translate-x-1/2 translate-x-4 px-1 py-px rounded-sm bg-[#00F5D4] text-[7px] font-bold text-black">AI</span>
                  )}
                  <Icon size={22} strokeWidth={1.6} className="text-white" />
                  <span className="text-[10px] text-white/85 text-center leading-tight px-1">{a.label}</span>
                </button>
              );
            })}
          </div>
        ...
        {cats.map(cat => {
          ...
          return (
            <button
              key={cat.id}
              onClick={() => {
                if (cat.actions.length === 1) { cat.actions[0].run(); setOpenCat(null); return; }
                setOpenCat(isOpen ? null : cat.id);
              }}
              className="shrink-0 flex flex-col items-center justify-center gap-1 px-4 min-w-[68px]"
              style={{ minHeight: 56 }}
            >
              <Icon size={22} strokeWidth={1.7} className={isOpen ? "text-[#00F5D4]" : "text-white"} />
              <span className={`text-[10px] ${isOpen ? "text-[#00F5D4] font-semibold" : "text-white/85"}`}>{cat.label}</span>
            </button>
          );
        })}
      </div>
