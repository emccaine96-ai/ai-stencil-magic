...
function ThumbTile({ src, preset, active, onPick }: {
  src: ImageData;
  preset: FilterPreset;
  active: boolean;
  onPick: () => void;
  ...
  return (
    <button
      ...
      <canvas ref={ref} className="block w-full h-auto rounded" style={{ imageRendering: "auto" }} />
      <div className="text-[10px] text-neutral-300 pb-0.5 truncate w-full text-center">{preset.name}</div>
      {active && (
        <div className="absolute top-1 right-1 bg-[#A855F7] rounded-full p-0.5"><Check size={10} /></div>
      )}
    </button>
  );
}
      ...
      <div className="rounded-xl border border-white/10 bg-[#121216] w-[92vw] max-w-[560px] p-4">
        ...
        <div className="grid grid-cols-3 sm:grid-cols-4 gap-2 max-h-[60vh] overflow-y-auto pr-1">
          {FILTER_PRESETS.map(p => (
            <ThumbTile key={p.id} src={thumb} preset={p} active={selected === p.id}
