...
export function EffectsPreviewModal({ source, preset, onApply, onClose }: Props) {
  ...
  useEffect(() => {
    setAmount(preset?.default ?? 0);
  }, [preset]);

  useEffect(() => {
    const c = canvasRef.current;
    if (!c || !thumb || !preset) return;
    c.width = thumb.width;
    ...
    try {
      preset.apply(clone, amount);
    } catch (e) {
      console.error("[fx-preview]", e);
    }
      ...
      <div className="rounded-xl border border-white/10 bg-[#121216] w-[92vw] max-w-[480px] p-4">
        <div className="flex items-center gap-2 mb-3">
          ...
          <div className="ml-auto text-[11px] text-neutral-400 tabular-nums">
            {amount.toFixed(preset.step < 1 ? 2 : 0)}
            {preset.unit ?? ""}
          </div>
        ...
        <div
          className="rounded-lg overflow-hidden bg-black/40 border border-white/5 flex items-center justify-center mb-3"
          style={{ minHeight: 180 }}
        >
