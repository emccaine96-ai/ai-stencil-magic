...
export function Timeline({
  ...
  useEffect(() => {
    ...
    let idx = Math.max(
      0,
      anim.frames.findIndex((f) => f.id === anim.activeFrameId),
    );
  ...
  function addFrame() {
    ...
    const f: Frame = {
      ...
      name: `Frame ${anim.frames.length + 1}`,
  ...
  function duplicateFrame(id: string) {
    const i = anim.frames.findIndex((f) => f.id === id);
    if (i < 0) return;
  ...
  async function doExport() {
    ...
    try {
      ...
      for (const fr of anim.frames) {
        ...
        tmp.width = composedCanvas.width;
        tmp.height = composedCanvas.height;
    ...
    <div className="border-t border-border bg-card/60 backdrop-blur px-2 py-2 flex flex-col gap-2">
      <div className="flex items-center gap-2 text-xs">
        ...
        <button
          ...
          disabled={anim.frames.length < 2}
          className="p-1 rounded hover:bg-muted disabled:opacity-50"
          aria-label="Play"
        >
          {playing ? <Pause size={13} /> : <Play size={13} />}
        </button>
        <button onClick={addFrame} className="p-1 rounded hover:bg-muted" aria-label="Add frame">
          <Plus size={13} />
        </button>
        <label className="flex items-center gap-1">
          ...
          <input
            ...
            value={anim.fps}
            onChange={(e) =>
              onChange({ ...anim, fps: Math.max(1, Math.min(60, Number(e.target.value) || 12)) })
            }
        ...
        <label className="flex items-center gap-1">
          ...
          <input
            ...
            max={3}
            value={onionSkin}
            onChange={(e) => setOnionSkin(Number(e.target.value))}
          />
        </label>
        <label className="flex items-center gap-1">
          <input
        ...
        <button
          onClick={doExport}
          disabled={busy || anim.frames.length < 1}
          className="inline-flex items-center gap-1 px-2 py-1 rounded bg-primary text-primary-foreground disabled:opacity-50"
        >
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        {anim.frames.map((f, i) => (
          <div
            key={f.id}
            className={`relative shrink-0 w-16 h-16 rounded border ${f.id === anim.activeFrameId ? "border-primary ring-1 ring-primary" : "border-border"} bg-background overflow-hidden cursor-pointer group`}
            onClick={() => {
              onChange({ ...anim, activeFrameId: f.id });
              onSelectFrame(f);
            }}
          >
            ...
            <span className="absolute top-0 left-0 px-1 text-[9px] bg-black/60 text-white">
              {i + 1}
            </span>
            <div className="absolute bottom-0 right-0 hidden group-hover:flex gap-0.5">
              <button
                ...
                className="p-0.5 bg-black/70 text-white hover:bg-primary"
              >
                <Copy size={9} />
              </button>
              <button
                ...
                className="p-0.5 bg-black/70 text-white hover:bg-destructive"
              >
                <Trash2 size={9} />
              </button>
            </div>
        ...
        {anim.frames.length === 0 && (
          <div className="text-[10px] text-muted-foreground italic">
            Click + to capture the current canvas as frame 1.
          </div>
        )}
      </div>
    </div>
