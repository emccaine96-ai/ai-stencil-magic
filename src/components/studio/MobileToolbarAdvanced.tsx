  ...
  return (
    <div className="md:hidden fixed bottom-0 inset-x-0 z-40 border-t border-border bg-background/95 backdrop-blur pb-[env(safe-area-inset-bottom)] flex flex-col gap-1 px-2 py-1">
      ...
      <div className="grid grid-cols-6 gap-1">
        <button type="button" className={btn} onClick={p.onPickBrush} data-active={p.active === "brush"}>
          <Brush className="w-5 h-5" /> Brush
        </button>
        <button type="button" className={btn} onClick={p.onPickEraser} data-active={p.active === "eraser"}>
      ...
      <div className="flex items-center gap-3 px-1">
        <label htmlFor="brushSize" className="text-[11px] w-10 text-muted-foreground">Size</label>
        <input
          id="brushSize"
          type="range"
          min={1}
      ...
      <div className="flex items-center gap-3 px-1">
        <label htmlFor="brushFlow" className="text-[11px] w-10 text-muted-foreground">Flow</label>
        <input
          id="brushFlow"
          type="range"
          min={0.01}
          ...
          className="flex-grow accent-primary"
        />
        <span className="w-10 text-[11px] text-right tabular-nums">{Math.round(p.brushFlow * 100)}%</span>
      </div>
    </div>
  );
}
