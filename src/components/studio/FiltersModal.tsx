      ...
      <div className="w-full max-w-2xl bg-card border border-border rounded-xl shadow-2xl overflow-hidden">
        ...
        <div className="grid md:grid-cols-2 gap-0">
          <div className="p-3 bg-muted/30 grid place-items-center min-h-[260px]">
            <canvas
              ref={previewRef}
              className="max-w-full max-h-[280px] rounded shadow border border-border bg-white"
            />
          </div>
          <div className="p-3 text-xs space-y-2.5">
            <div className="grid grid-cols-2 gap-1">
              {Filters.map((f) => (
                <button
                  key={f}
                  onClick={() => setFilter(f)}
                  className={`py-1.5 rounded text-[11px] font-semibold ${filter === f ? "bg-primary text-primary-foreground" : "bg-muted"}`}
                >
                  {FILTER_LABELS[f]}
                </button>
              ))}
            ...
            <div className="pt-2 border-t border-border space-y-2">
              ...
              {filter === "sharpen" && (
                <FSlider
                  label="Amount"
                  v={params.sharpenAmount}
                  min={0}
                  max={200}
                  onChange={(v) => setParams((p) => ({ ...p, sharpenAmount: v }))}
                />
              )}
              {(filter === "invert" || filter === "grayscale") && (
                <p className="text-muted-foreground text-[11px]">
                  No parameters — preview shows the result.
                </p>
              )}
            </div>
          </div>
        ...
        <footer className="px-4 py-3 border-t border-border flex justify-end gap-2">
          <button onClick={onCancel} className="px-3 py-1.5 rounded text-xs hover:bg-muted">
            Cancel
          </button>
          <button
            onClick={commit}
  ...
  return (
    <label className="block">
      <div className="flex justify-between text-[10px] text-muted-foreground">
        <span>{label}</span>
        <span className="tabular-nums">{Math.round(v * 10) / 10}</span>
      </div>
      <input
        type="range"
        min={min}
