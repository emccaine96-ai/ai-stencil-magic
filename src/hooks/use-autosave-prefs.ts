...
export function useAutosavePrefs() {
  ...
  const [intervalMs, setIntervalMsState] = useState<number>(() => readNum(KEY_INTERVAL, DEFAULT_INTERVAL_MS));

  const setEnabled = useCallback((v: boolean) => {
    setEnabledState(v);
    try { window.localStorage.setItem(KEY_ENABLED, v ? "1" : "0"); } catch { /* ignore */ }
  }, []);

  const setIntervalMs = useCallback((ms: number) => {
    setIntervalMsState(ms);
    try { window.localStorage.setItem(KEY_INTERVAL, String(ms)); } catch { /* ignore */ }
  }, []);
