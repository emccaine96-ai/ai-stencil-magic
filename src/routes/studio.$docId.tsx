  ...
  head: () => ({
    meta: [
      { title: "Canvas Studio — AI Stencil Magic" },
      {
        name: "description",
        content:
          "Procreate-inspired stencil editor: pressure-sensitive brushes, layers, selections, free transform.",
      },
    ],
  }),
...
type FloatingTransform = {
  ...
  tx: number;
  ty: number; // additional translation
  scale: number;
...
function StudioPage() {
  ...
  const drawOverlay = useCallback(() => {
    ...
    if (floating) {
      ...
      ctx.beginPath();
      ctx.moveTo(0, -hh);
      ctx.lineTo(0, -hh - 40 / f.scale);
      ctx.stroke();
      ctx.beginPath();
      ctx.arc(0, -hh - 40 / f.scale, hs * 0.7, 0, Math.PI * 2);
      ctx.fill();
      ctx.restore();
    }
  ...
  function onPointerMove(e: React.PointerEvent<HTMLCanvasElement>) {
    ...
    for (const ev of evts) {
      ...
      const rawPressure =
        (ev as PointerEvent).pressure > 0 && (ev as PointerEvent).pointerType !== "mouse"
      ...
      const mp = mirroredPoints({ x: sp.x, y: sp.y }, symmetry, CANVAS_W, CANVAS_H);
      mp.forEach((q, i) => {
        if (s.scs[i]) strokeTo(s.scs[i], q.x, q.y, sp.pressure);
      });
      s.prev = sp;
      setPressure(sp.pressure);
    }
  ...
  function onPointerUp(e: React.PointerEvent<HTMLCanvasElement>) {
    if (tfmDragRef.current) {
      tfmDragRef.current = null;
      return;
    }
    ...
    if (layer) {
      ...
      if (lc) {
        ...
        if (s.target !== lc && scratchRef.current) {
          ...
          scratchRef.current
            .getContext("2d")!
            .clearRect(0, 0, scratchRef.current.width, scratchRef.current.height);
        }
  ...
  function applySelectOp(prev: SelectionMask, op: SelectOp): SelectionMask {
    ...
    (m as SelectionMask & { _baseSub?: Uint8ClampedArray })._baseSub = new Uint8ClampedArray(
      prev.data,
    );
    return m;
  }
  function finalizeMask(m: SelectionMask) {
    ...
    let any = false;
    for (let i = 0; i < m.data.length; i += 64)
      if (m.data[i]) {
        any = true;
        break;
      }
  ...
  function copySelection() {
    ...
    const lc = layerCanvases.current.get(layer.id);
    if (!lc) return;
    const bounds = maskBounds(maskRef.current);
    if (!bounds) return;
  ...
  function cutSelection() {
    ...
    historyRef.current.push({
      layerId: layer.id,
      before,
      after: lc.getContext("2d")!.getImageData(0, 0, lc.width, lc.height),
    });
    setDirty(true);
    compose();
  }
  async function pasteClipboard() {
    ...
    c.getContext("2d")!.drawImage(
      ...
      (state.width - clipboardRef.current.width) / 2,
      (state.height - clipboardRef.current.height) / 2,
    );
  ...
  function commitTransform() {
    ...
    const layer = state.layers.find((l) => l.id === floating.layerId);
    if (!layer) return;
    const lc = layerCanvases.current.get(layer.id);
    if (!lc) return;
    ...
    historyRef.current.push({
      layerId: layer.id,
      before,
      after: ctx.getImageData(0, 0, lc.width, lc.height),
    });
    setFloating(null);
    setTool("brush");
    setDirty(true);
  ...
  function undo() {
    ...
    if (!h) return;
    const lc = layerCanvases.current.get(h.layerId);
    if (!lc) return;
    ...
    async (changes: string) => {
      ...
      try {
        const layers = state.layers.map((l) => ({
          ...l,
          dataUrl: layerCanvases.current.get(l.id)?.toDataURL("image/png") ?? "",
        }));
  ...
  useEffect(() => {
    function onKey(e: KeyboardEvent) {
      if ((e.metaKey || e.ctrlKey) && e.key === "z") {
        ...
        e.preventDefault();
        startTransform();
      } else if (e.key === "Enter" && floating) {
        e.preventDefault();
        commitTransform();
      } else if (e.key === "Escape" && floating) {
        e.preventDefault();
        cancelTransform();
      }
    }
  ...
  function flattenVisible() {
    ...
    for (const l of visible) {
      const lc = layerCanvases.current.get(l.id);
      if (!lc) continue;
  ...
  return (
    <div className="min-h-screen h-screen flex flex-col bg-background text-foreground overflow-hidden touch-none">
      ...
      <header className="shrink-0 bg-card border-b border-border h-12 flex items-center px-2 gap-1 sm:gap-2 text-xs">
        ...
        />
        <button onClick={undo} className="p-1.5 rounded hover:bg-muted" aria-label="Undo">
          <Undo2 size={14} />
        </button>
        <button onClick={redo} className="p-1.5 rounded hover:bg-muted" aria-label="Redo">
          <Redo2 size={14} />
        </button>
        <button
          onClick={() => save("Manual save")}
          className="px-2 py-1 rounded bg-gradient-primary text-primary-foreground font-semibold flex items-center gap-1"
        >
      ...
      <div className="shrink-0 bg-card/70 border-b border-border px-2 py-1.5 flex items-center gap-1 text-[11px] overflow-x-auto">
        ...
        {(tool === "rect" || tool === "ellipse" || tool === "lasso" || tool === "wand") && (
          <>
            <select
              value={selectOp}
              onChange={(e) => setSelectOp(e.target.value as SelectOp)}
        ...
        <ToolBtn
          ...
          icon={<X size={13} />}
          label="Deselect"
          disabled={!hasSelection && !floating}
        />
        <ToolBtn
        ...
        {floating && (
          <>
            <ToolBtn
              onClick={() => setFloating({ ...floating, flipX: !floating.flipX })}
              icon={<FlipHorizontal size={13} />}
              label="Flip H"
            />
            <ToolBtn
              onClick={() => setFloating({ ...floating, flipY: !floating.flipY })}
            ...
            <button
              ...
              className="px-2 py-1 rounded bg-muted font-semibold flex items-center gap-1"
            >
              <X size={13} /> Cancel
            </button>
          </>
      ...
      <div className="flex-1 min-h-0 flex">
        ...
        <div
          ref={wrapRef}
          className="flex-1 min-w-0 relative bg-muted/30 overflow-auto grid place-items-center p-2"
        >
        ...
        <aside className="hidden lg:flex w-72 border-l border-border bg-card flex-col overflow-y-auto">
          <BrushPanel
            brush={brush}
            setOverrides={setBrushOverrides}
            color={color}
            setColor={setColor}
            smoothing={smoothing}
      ...
      <div className="lg:hidden shrink-0 border-t border-border bg-card">
        <div className="flex">
          <button
            ...
            className={`flex-1 py-2.5 text-xs font-semibold flex items-center justify-center gap-1.5 ${showBrushes ? "text-primary" : ""}`}
          >
          ...
          <button
            ...
            className={`flex-1 py-2.5 text-xs font-semibold flex items-center justify-center gap-1.5 ${showLayers ? "text-primary" : ""}`}
          >
            ...
            <div className="grid grid-cols-3 gap-1 p-2">
              {(Object.keys(DEFAULTS) as BrushId[]).map((id) => (
                <button
                  ...
                  className={`py-2 rounded text-[11px] font-semibold ${activeBrushId === id ? "bg-primary text-primary-foreground" : "bg-muted"}`}
                >
                  {BRUSH_LABELS[id]}
                </button>
              ))}
      ...
      {exportOpen && composedRef.current && (
        <ExportModal
          canvas={composedRef.current}
          defaultName={doc.name || "stencil"}
    ...
    <div className="p-3 border-b border-border space-y-2.5 text-xs">
      <div className="flex items-center justify-between">
        <span className="font-bold uppercase tracking-wider text-[10px] text-muted-foreground">
          {BRUSH_LABELS[brush.id]}
        </span>
        <input
          type="color"
          value={color}
          onChange={(e) => setColor(e.target.value)}
          className="h-7 w-9 rounded cursor-pointer bg-transparent border border-border"
        />
      </div>
      <Slider
        label="Size"
      ...
      <Slider
        label="Pressure → Size"
        value={brush.pressureSize * 100}
        min={0}
        max={100}
        step={1}
        onChange={(v) => set("pressureSize", v / 100)}
        suffix="%"
      />
      <Slider
        label="Pressure → Opacity"
        value={brush.pressureOpacity * 100}
        min={0}
        max={100}
        step={1}
        onChange={(v) => set("pressureOpacity", v / 100)}
        suffix="%"
      />
      <Slider
        label="Pressure Curve"
        value={brush.pressureCurve * 100}
        min={30}
        max={300}
        step={5}
        onChange={(v) => set("pressureCurve", v / 100)}
        suffix="%"
      />
      <Slider
        label="Stabiliser"
  ...
  return (
    <div className="flex-1 p-2 text-xs">
      <div className="flex items-center justify-between p-1">
        ...
        <button onClick={addLayer} className="p-1 rounded hover:bg-muted" aria-label="Add layer">
          <Plus size={14} />
        </button>
      </div>
      <div className="space-y-1">
        {[...state.layers].reverse().map((layer) => {
          ...
          return (
            <div
              key={layer.id}
              className={`rounded-lg border ${active ? "border-primary bg-primary/5" : "border-border"} p-2`}
            >
                ...
                <div className="mt-1.5 space-y-1.5">
                  ...
                  <label className="flex items-center gap-1.5 text-[10px]">
                    <input
                      ...
                      onChange={(e) => updateLayer(layer.id, { alphaLock: e.target.checked })}
                    />{" "}
                    Alpha lock
                  </label>
                </div>
...
async function ensureEditorState(d: DocumentData): Promise<EditorState> {
  if (d.layeredEditorData) {
    try {
      return JSON.parse(d.layeredEditorData) as EditorState;
    } catch {
      /* fall through */
    }
  }
...
function maskAsCanvas(m: SelectionMask): HTMLCanvasElement {
  if (!_maskCanvas || _maskCanvas.width !== m.width) {
    ...
    _maskCanvas.width = m.width;
    _maskCanvas.height = m.height;
  }
  _maskCanvas.getContext("2d")!.putImageData(maskToImageData(m), 0, 0);
  return _maskCanvas;
}

function bakeScratchToLayer(
...
function makeMaskedPreview(
  scratch: HTMLCanvasElement,
  mask: SelectionMask,
): HTMLCanvasElement | null {
  // Build a temporary clone with mask applied for live preview only.
  const c = document.createElement("canvas");
  c.width = scratch.width;
  c.height = scratch.height;
  const ctx = c.getContext("2d")!;
...
function pickTransformHandle(f: FloatingTransform, p: { x: number; y: number }) {
  ...
  const dx = p.x - cx,
    dy = p.y - cy;
  const ca = Math.cos(-f.rotation),
    sa = Math.sin(-f.rotation);
