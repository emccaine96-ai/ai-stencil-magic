...
export function floodFill(
  ...
  const matches = (i: number) => {
    ...
    return Math.abs(dr) <= tol && Math.abs(dg) <= tol
        && Math.abs(db) <= tol && Math.abs(da) <= tol;
  };
  ...
  while (stack.length) {
    ...
    for (let rx = lx; rx < w; rx++) {
      ...
      if (cy > 0) {
        ...
        if (hit && !spanAbove) { stack.push(rx, cy - 1); spanAbove = true; }
        else if (!hit) spanAbove = false;
      }
      if (cy < h - 1) {
        const dn = (cy + 1) * w + rx;
        const hit = !visited[dn] && matches(dn * 4);
        if (hit && !spanBelow) { stack.push(rx, cy + 1); spanBelow = true; }
