...
export type Handle = "move" | "nw" | "n" | "ne" | "e" | "se" | "s" | "sw" | "w" | "rotate" | null;

export interface TransformState {
  x: number;
  y: number; // top-left in world coords
  w: number;
  h: number; // size (unscaled)
  scaleX: number;
  scaleY: number;
...
export class TransformController {
  ...
  private hit(px: number, py: number): Handle {
    ...
    const hs2 = hs * hs;
    const near = (h: { x: number; y: number }) => (px - h.x) ** 2 + (py - h.y) ** 2 < hs2;
  ...
  pointerMove(px: number, py: number): boolean {
    ...
    if (this.active === "move") {
      ...
      const cx = s0.x + s0.w / 2,
        cy = s0.y + s0.h / 2;
      const rx0 = this.start.px - cx,
        ry0 = this.start.py - cy;
      const rx1 = px - cx,
        ry1 = py - cy;
      ...
      if (/nw|ne|sw|se/.test(this.active)) {
        ...
        const u =
          (Math.abs(this.state.scaleX / s0.scaleX) + Math.abs(this.state.scaleY / s0.scaleY)) / 2;
