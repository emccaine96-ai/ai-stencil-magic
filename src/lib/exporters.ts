...
<svg xmlns="http://www.w3.org/2000/svg" width="${canvas.width}" height="${canvas.height}" viewBox="0 0 ${canvas.width} ${canvas.height}">
  <image href="${data}" width="${canvas.width}" height="${canvas.height}" />
      ...
      download(url, `${name}.svg`); setTimeout(() => URL.revokeObjectURL(url), 5000);
      return;
    }
  }
}
...
export async function exportTiledPrint(canvas: HTMLCanvasElement, o: TilePrintOptions) {
  const [pw, ph] = o.orientation === "landscape"
  ...
  await new Promise<void>((res) => {
    ...
    img.onload = () => {
      ...
      ctx.drawImage(img, 0, 0, full.width, full.height); res();
    };
    img.src = fullDataUrl;
  });
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      pdf.text(`Tile ${r + 1}-${c + 1}  ·  ${cols}×${rows}  ·  ${o.targetWidthIn}" wide`, margin, ph - margin / 2);
    }
  }
...
function sanitize(n: string) { return n.replace(/[^a-z0-9\-_]+/gi, "_").slice(0, 80) || "stencil"; }