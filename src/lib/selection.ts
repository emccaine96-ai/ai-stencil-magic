...
export function fillEllipse(m: SelectionMask, x0: number, y0: number, x1: number, y1: number) {
  ...
  const xa = Math.max(0, (cx - rx) | 0), xb = Math.min(m.width, (cx + rx + 1) | 0);
  const ya = Math.max(0, (cy - ry) | 0), yb = Math.min(m.height, (cy + ry + 1) | 0);
  for (let y = ya; y < yb; y++) {
    for (let x = xa; x < xb; x++) {
...
export function magicWand(
  m: SelectionMask, src: ImageData, sx: number, sy: number, tolerance: number, contiguous = true,
) {
...
export function maskBounds(m: SelectionMask): { x: number; y: number; w: number; h: number } | null {
  const w = m.width, h = m.height;
