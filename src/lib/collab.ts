...
export class CollabSession {
  ...
  constructor(public roomId: string, me: Omit<CollabPeer, "x" | "y">, private handlers: CollabHandlers = {}) {
    this.me = { ...me, x: 0, y: 0 };
  }

  async join() {
    const ch = supabase.channel(`studio:${this.roomId}`, { config: { presence: { key: this.me.id } } });
    this.channel = ch;
  ...
  sendStroke(stroke: VectorStroke) {
    this.channel?.send({ type: "broadcast", event: "stroke", payload: { stroke, from: this.me.id } });
  }
