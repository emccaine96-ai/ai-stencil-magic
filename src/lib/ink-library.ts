...
export function kmeansColors(
  ...
  const sampleStride = Math.max(1, Math.floor(Math.sqrt((imageData.width * imageData.height) / 12000)));
  ...
  for (let it = 0; it < maxIter; it++) {
    ...
    for (let i = 0; i < samples.length; i++) {
      ...
      sums[c][0] += s[0]; sums[c][1] += s[1]; sums[c][2] += s[2]; sums[c][3]++;
    }
    for (let c = 0; c < centroids.length; c++) {
...
export function mixRecipe(targetHex: string): { name: string; hex: string; pct: number }[] {
  ...
  for (let it = 0; it < 220; it++) {
    ...
    for (let i = 0; i < N; i++) {
      ...
      if (w[i] < 0) w[i] = 0;
      const down = cost();
      if (down <= up && down < base) { /* keep */ improved = true; }
      else if (up < base) { w[i] += 2 * step; improved = true; }
      else { w[i] += step; }
    }
    if (!improved) break;
  }
