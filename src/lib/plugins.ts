...
export const BUILTIN_PLUGINS: Plugin[] = [
  ...
  },
  {
    ...
    author: "AI Stencil Magic",
    kind: "filter",
    description: "Bridges small gaps in broken lines — morphological close operation to connect stencil edges.",
    params: [
    ...
    code: [
      ...
      "var dil=new Uint8ClampedArray(d.length);",
      "for(var y=0;y<h;y++)for(var x=0;x<w;x++){",
      "  var dark=false;",
      "  for(var dy=-r;dy<=r;dy++)for(var dx=-r;dx<=r;dx++){",
      "    var nx=x+dx,ny=y+dy;",
      "    if(nx<0||nx>=w||ny<0||ny>=h)continue;",
      "    if(d[(ny*w+nx)*4]<128){dark=true;break;}",
      "  }",
      "  var i=(y*w+x)*4;",
      "  dil[i]=dark?0:255;dil[i+1]=dark?0:255;dil[i+2]=dark?0:255;dil[i+3]=255;",
      "}",
      "for(var j=0;j<d.length;j++)d[j]=dil[j];",
    ].join("\n"),
  },
  {
  ...
  },
  {
    ...
    code: [
      ...
      "for(var y=0;y<h;y+=sp)for(var x=0;x<w;x+=sp){",
      "  var i=(y*w+x)*4;",
      "  var lum=0.3*src[i]+0.59*src[i+1]+0.11*src[i+2];",
      "  var darkness=1-lum/255;",
      "  var r=Math.round(darkness*mr);",
      "  if(r<1)continue;",
      "  for(var dy=-r;dy<=r;dy++)for(var dx=-r;dx<=r;dx++){",
      ...
      "    if(nx<0||nx>=w||ny<0||ny>=h)continue;",
      "    var j=(ny*w+nx)*4;",
      "    d[j]=0;d[j+1]=0;d[j+2]=0;d[j+3]=255;",
      "  }",
      "}",
    ].join("\n"),
  },
  ...
  },
  {
    ...
    author: "AI Stencil Magic",
    kind: "filter",
    description: "Creates a mirrored left-right copy — perfect for symmetrical designs (mandalas, butterflies, etc).",
    params: [
      { key: "axis", label: "Axis", type: "string", default: "horizontal", min: 0, max: 0, step: 0 },
    ],
    code: [
  ...
  },
  {
    ...
    code: [
      "var w=ctx.width,h=ctx.height,d=ctx.data,r=ctx.params.radius|0,th=ctx.params.threshold|0;",
      "var out=new Uint8ClampedArray(d.length);",
      "for(var y=0;y<h;y++)for(var x=0;x<w;x++){",
      "  var i=(y*w+x)*4;",
