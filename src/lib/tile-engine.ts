...
export class TileEngine {
  ...
  static key(tx: number, ty: number): TileKey {
    return tx + "," + ty;
  }
  ...
  hasTile(tx: number, ty: number) {
    return this.tiles.has(TileEngine.key(tx, ty));
  }
  ...
  composite(
    ...
    const docX = viewX / scale,
      docY = viewY / scale;
    const docW = viewW / scale,
      docH = viewH / scale;
