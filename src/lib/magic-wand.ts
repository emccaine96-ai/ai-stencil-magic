  ...
  if (!contiguous) {
    ...
    while (stack.length) {
      ...
      while (lx < w && !mask[y * w + lx] && match(at(lx, y))) {
        mask[y * w + lx] = 255;
        if (lx < x0) x0 = lx; if (y < y0) y0 = y;
        if (lx > x1) x1 = lx; if (y > y1) y1 = y;
        if (y > 0) {
          const a = match(at(lx, y - 1)) && !mask[(y - 1) * w + lx];
          if (!above && a) { stack.push(lx, y - 1); above = true; }
          else if (above && !a) above = false;
        }
        if (y < h - 1) {
          const b = match(at(lx, y + 1)) && !mask[(y + 1) * w + lx];
          if (!below && b) { stack.push(lx, y + 1); below = true; }
...
function morph(mask: Uint8Array, w: number, h: number, passes: number, dilate: boolean): Uint8Array {
  let cur = mask;
...
export function maskToOverlayCanvas(mask: Uint8Array, w: number, h: number, hex = "#00F5D4"): HTMLCanvasElement {
