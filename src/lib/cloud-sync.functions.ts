...
export const pushDocument = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { error, data: row } = await supabase
      ...
      .upsert(
        { user_id: userId, local_id: data.localId, name: data.name, thumbnail: data.thumbnail ?? null, payload: data.payload as any },
...
export const pullDocument = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { supabase } = context;
    const { data: row, error } = await supabase.from("cloud_documents").select("*").eq("id", data.id).single();
    if (error) throw new Error(error.message);
    return row;
  });
...
export const setShareToken = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: unknown) => z.object({ id: z.string().uuid(), enable: z.boolean() }).parse(input))
  .handler(async ({ data, context }) => {
    const { supabase } = context;
