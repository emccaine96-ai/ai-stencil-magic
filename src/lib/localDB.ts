...
export async function createDocument(input: Partial<DocumentData> & { thumbnail: string }): Promise<DocumentData> {
  const now = Date.now();
...
export async function saveDocument(doc: DocumentData, snapshot?: { changes: string; thumbnail?: string; editorState?: string }): Promise<DocumentData> {
  const d = await db();
  if (!d) return doc;
  const now = Date.now();
  ...
  if (snapshot) {
    const hist = [
      ...doc.versionHistory,
      { timestamp: now, thumbnail: snapshot.thumbnail ?? doc.thumbnail, changes: snapshot.changes, editorState: snapshot.editorState },
    ];
...
export async function importBackup(bundle: BackupBundle, opts: { merge?: boolean } = {}): Promise<{ documents: number; folders: number }> {
