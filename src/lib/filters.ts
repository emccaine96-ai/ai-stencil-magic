...
export function applyFilter(
  src: HTMLCanvasElement, id: FilterId, p: FilterParams, mask?: Uint8ClampedArray,
): HTMLCanvasElement {
  ...
  if (id === "blur") {
    ...
    switch (id) {
      case "brightness-contrast": brightnessContrast(img, p.brightness, p.contrast); break;
      case "hsl":                  hsl(img, p.hue, p.saturation, p.lightness); break;
      case "invert":               invert(img); break;
      case "threshold":            threshold(img, p.threshold); break;
      case "sharpen":              sharpen(img, p.sharpenAmount / 100); break;
      case "grayscale":            grayscale(img); break;
    }
...
function compositeWithMask(orig: HTMLCanvasElement, filtered: HTMLCanvasElement, mask: Uint8ClampedArray) {
  const fctx = filtered.getContext("2d")!;
  const w = filtered.width, h = filtered.height;
