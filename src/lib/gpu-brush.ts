...
export class GpuBrushRenderer {
  ...
  private init() {
    ...
    gl.enableVertexAttribArray(aPos);   gl.vertexAttribPointer(aPos,   2, gl.FLOAT, false, stride, 0);  gl.vertexAttribDivisor(aPos, 1);
    gl.enableVertexAttribArray(aSize);  gl.vertexAttribPointer(aSize,  1, gl.FLOAT, false, stride, 8);  gl.vertexAttribDivisor(aSize, 1);
    gl.enableVertexAttribArray(aAngle); gl.vertexAttribPointer(aAngle, 1, gl.FLOAT, false, stride, 12); gl.vertexAttribDivisor(aAngle, 1);
    gl.enableVertexAttribArray(aAlpha); gl.vertexAttribPointer(aAlpha, 1, gl.FLOAT, false, stride, 16); gl.vertexAttribDivisor(aAlpha, 1);
  ...
  setBrush(brush: BrushSettings) {
    ...
    this.gl.texImage2D(this.gl.TEXTURE_2D, 0, this.gl.RGBA, this.gl.RGBA, this.gl.UNSIGNED_BYTE, stamp);
  }
  ...
  drawStamps(stamps: StampInstance[]) {
    ...
    for (let i = 0; i < stamps.length; i++) {
      const s = stamps[i]; const o = i * 5;
      data[o] = s.x; data[o+1] = s.y; data[o+2] = s.size; data[o+3] = s.angle; data[o+4] = s.alpha;
    }
    gl.bindBuffer(gl.ARRAY_BUFFER, this.instBuf);
