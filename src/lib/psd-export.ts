...
const BLEND_MAP: Record<LayerBlendMode, string> = {
  ...
  "hard-light": "hLit", "soft-light": "sLit", difference: "diff",
  exclusion: "smud", hue: "hue ", saturation: "sat ", color: "colr", luminosity: "lum ",
};

class Writer {
  parts: Uint8Array[] = [];
  push(...bytes: number[]) { this.parts.push(new Uint8Array(bytes)); }
  u16(v: number) { this.parts.push(new Uint8Array([(v >> 8) & 0xff, v & 0xff])); }
  u32(v: number) { this.parts.push(new Uint8Array([(v >> 24) & 0xff, (v >> 16) & 0xff, (v >> 8) & 0xff, v & 0xff])); }
  i32(v: number) { this.u32(v >>> 0); }
  i16(v: number) { this.u16(v & 0xffff); }
  ascii(s: string) { this.parts.push(new TextEncoder().encode(s)); }
  raw(a: Uint8Array) { this.parts.push(a); }
  pascalPad4(s: string) {
...
function planarRGBA(canvas: HTMLCanvasElement): { r: Uint8Array; g: Uint8Array; b: Uint8Array; a: Uint8Array } {
...
export function exportPSD(width: number, height: number, layers: Layer[], composited: HTMLCanvasElement): Blob {
  const w = new Writer();
  // === File Header ===
  w.ascii("8BPS"); w.u16(1); w.raw(new Uint8Array(6));
  w.u16(4); w.u32(height); w.u32(width); w.u16(8); w.u16(3); // RGB, 8bpc, 4 channels (RGBA)
  ...
  for (const layer of rasterLayers) {
    ...
    layerInfo.u32(0); layerInfo.u32(0); layerInfo.u32(c.height); layerInfo.u32(c.width);
    ...
    for (const ch of channels) {
      ...
      layerInfo.i16(ch.id); layerInfo.u32(len);
