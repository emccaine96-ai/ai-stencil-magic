...
function getWorker(): Worker | null {
  ...
  try {
    ...
    _worker.onmessage = (e: MessageEvent<{ id: number; ok: boolean; data?: ImageData; error?: string }>) => {
      const { id, ok, data, error } = e.data;
      const okCb = _pending.get(id); const failCb = _failed.get(id);
      _pending.delete(id); _failed.delete(id);
...
function fallback(op: Op): ImageData {
  const w = op.data.width, h = op.data.height;
  ...
  if (op.op === "threshold" || op.op === "otsu") {
    ...
    for (let i = 0; i < d.length; i += 4) { const v = Y(i) < level ? 0 : 255; d[i] = d[i + 1] = d[i + 2] = v; }
  } else if (op.op === "thermal-blue") {
    const C = { r: 0x2b, g: 0x3a, b: 0x8c }, K = { r: 0xfa, g: 0xf6, b: 0xea };
