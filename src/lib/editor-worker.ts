...
type Op =
  ...
  | { id: number; op: "morph"; data: ImageData; passes: number; kind: "erode" | "dilate" | "open" | "close" }
...
function morph(data: ImageData, kind: "erode" | "dilate" | "open" | "close", passes: number): ImageData {
  let cur = data;
  const seq: ("erode" | "dilate")[] =
    kind === "open" ? ["erode", "dilate"]
    : kind === "close" ? ["dilate", "erode"]
    : [kind];
...
self.onmessage = (e: MessageEvent<Op>) => {
  ...
  try {
    ...
    switch (msg.op) {
      case "threshold":     result = threshold(msg.data, msg.level); break;
      case "otsu":          result = threshold(msg.data, otsuLevel(msg.data)); break;
      case "morph":         result = morph(msg.data, msg.kind, Math.max(1, msg.passes)); break;
      case "thermal-blue":  result = thermalBlue(msg.data); break;
      case "thermal-purple":result = thermalPurple(msg.data); break;
      case "stipple":       result = stipple(msg.data, msg.density, msg.size); break;
    }
    (self as unknown as Worker).postMessage(
      { id: msg.id, ok: true, data: result },
      [result.data.buffer]
    );
  } catch (err) {
