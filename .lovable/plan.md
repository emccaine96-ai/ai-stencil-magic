...
- `src/components/vault/DrawModeOverlay.tsx` _(new)_ — thin wrapper that mounts the existing Procreate left/right drawers when Draw mode is active.
- `src/components/vault/CropOverlay.tsx` _(new)_ — interactive crop rect with aspect presets.
- `src/lib/picsart-filters.ts` — fill in any missing ops, ensure each returns/commits cleanly.
- `src/components/vault/AutosaveToggle.tsx` _(new)_ — top-bar pill + long-press popover.
